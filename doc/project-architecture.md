...
4. `total_cost` 对 23 个 `(m,n)` 级次逐一执行固定物理前向：
   ...
   - `Uc = exp(iΦc)`
   - `Ic = |fftshift(FFT2(Uc))|²`
5. 基础损失综合目标图 MSE、目标区域能量效率和通道权重；阶段 D 在此基础上可选约束公共灰度比例、同档跨通道一致性、档位间隔、线内均匀性、最低档可见性、最差灰度档以及背景像素/逐行均匀性。
...
阶段 D 已完成损失接口、背景逐行均匀性指标和多组短轮候选实验；当前候选 007 仍需正式长跑和第二随机种子复核。Python 主流程输出 `phdx/phdy` 到 NPZ，而 MATLAB 版图脚本读取 CSV，两者之间缺少正式导出步骤。辅助脚本还含固定的本机路径。
