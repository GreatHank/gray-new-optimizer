...
- 阶段 C 已完成一次 30000 轮固定比例基线，但这不等同于 FDTD/器件验证，也不代表灰度比例已经达标。

## 当前状态与下一步

项目已完成目录归类、架构梳理和主流程最小运行验证。当前处于 `experiment/four-level-lines` 分支的阶段 C：已使用原有图像损失完成 30000 轮四级三线固定比例基线，结果保存在 `output/baseline_four_level_lines`；原损失使 23 个通道的三档响应保持单调，但未严格达到 `1/3、2/3、1` 比例。下一步是阶段 D，仅增加灰度相关损失，完成后仍需先由用户复核统一曝光图，再考虑水果轮廓实验。四级灰度的核心需求和实施计划已确认。
