...
class ThinLetterTargetTests(unittest.TestCase):
    ...
    def test_empty_grid_cells_are_excluded(self):
        ...
        for row, column in active_cells:
            row0 = row * 100 + 20
            column0 = column * 100 + 20
            gray[row0 : row0 + 10, column0 : column0 + 60] = 85
            gray[row0 + 10 : row0 + 20, column0 : column0 + 60] = 170
            gray[row0 + 20 : row0 + 30, column0 : column0 + 60] = 255
        gray[0, 0] = 255
        gray[0, -1] = 255
        gray[-1, 0] = 255

        targets, _tiles, selected = target_generator.build_targets(
            gray,
            channel_count=36,
            background_delta=1,
            min_line_intensity=1,
            min_tile_line_pixels=30,
        )

        self.assertEqual(targets.shape[0], 31)
        self.assertEqual(
            [(item[0], item[1]) for item in selected],
            active_cells,
        )

    def test_fixed_assignment_preserves_missing_source_levels(self):
        ...
        target = target_generator.quantize_tile(
            ...
            background_delta=1,
            min_line_intensity=1,
            level_assignment="fixed",
        )

        np.testing.assert_allclose(
            np.unique(target), np.array([0.0, 1 / 3], dtype=np.float32)
        )
