...
def quantize_tile(
    ...
    elif level_assignment == "fixed":
        quantized[mask & (resized < 127.5)] = LEVELS[0]
        quantized[mask & (resized >= 127.5) & (resized < 212.5)] = LEVELS[1]
        quantized[mask & (resized >= 212.5)] = LEVELS[2]
...
def build_targets(
    ...
    eligible = [item for item in tiles if item[4] >= min_tile_line_pixels]
    if not eligible:
        raise ValueError("6x6网格中没有达到最低线条像素数的有效区域。")
    selected = sorted(
        eligible, key=lambda item: (-item[4], item[0], item[1])
    )[:channel_count]
...
def save_grid_preview(tiles, selected, output_file):
    ...
    for grid_row, grid_column, _bounds, tile, score in tiles:
        ...
        axis.axis("off")
    figure.suptitle(
        f"6 x 6 source crops; {len(selected)} active physical channels"
    )
    figure.tight_layout()
...
def parse_args():
    ...
    parser.add_argument(
        "--level-assignment",
        choices=("source", "fixed", "spatial"),
        default="source",
        help=(
            "source按块内亮度分位分级；fixed按85/170/255固定灰度分级；"
            "spatial沿线条空间位置连续分成三档。"
        ),
    )
    parser.add_argument(
        "--min-tile-line-pixels",
        type=int,
        default=30,
        help="线条像素少于该值的网格不参与优化；用于彻底排除纯黑channel。",
    )
...
def main():
    ...
    if args.min_tile_line_pixels < 1:
        raise ValueError("min-tile-line-pixels必须大于0。")
    targets, tiles, selected = build_targets(
        ...
        min_tile_line_pixels=args.min_tile_line_pixels,
    )

    mat_file = args.output_dir / "thin_letter_6x6_target.mat"
    grid_positions = np.asarray(
        [[item[0], item[1]] for item in selected], dtype=np.int16
    )
    sio.savemat(
        mat_file,
        {"bw_all": targets, "grid_positions": grid_positions},
        do_compression=True,
    )
